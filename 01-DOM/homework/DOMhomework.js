...
// Tu código acá:
document.querySelector('#createdBy').innerHTML = 'Aplicacion creada por Marian';
...
function buildToDos(toDos) {
  ...
  let index = 0
  toDos.map((element)=>{
    array.push(buildToDo(element,index))
    index ++
  })
...
function displayToDos() {
  // Tu código acá:
  toDoCointainer = document.getElementById('toDoContainer')
  toDoCointainer.innerHTML = ''
  buildToDos(toDoItems).forEach(e => {
    toDoCointainer.append(e)
  })
}
...
function addToDo() {
  // Tu código acá:
  let texto = document.getElementById('toDoInput')
  let toDo = new ToDo(texto.value)
  toDoItems.push(toDo)
  texto.value = ''
  displayToDos()
}
